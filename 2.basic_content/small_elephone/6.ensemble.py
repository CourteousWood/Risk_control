...
best_xgb = XGBClassifier(learning_rate =0.1, n_estimators=50, objective= 'binary:logistic',nthread=40,scale_pos_weight=0.6,seed=27)
...
xgb_pred = [1 if i > 0.5 else 0 for i in xgb_pred ]
...
print(classification_report(trainData['target'], xgb_pred))
print(confusion_matrix(trainData['target'], xgb_pred))
print(accuracy_score(trainData['target'], xgb_pred))
print(roc_auc_score(trainData['target'], xgb_pred))
...
ann_pred = [1 if i > 0.5 else 0 for i in ann_pred ]
...
print(classification_report(mydata['target'], ann_pred))
print(confusion_matrix(mydata['target'], ann_pred))
print(accuracy_score(mydata['target'], ann_pred))
print(roc_auc_score(mydata['target'], ann_pred))
...
gsearch = GridSearchCV(estimator = AdaBoostClassifier(),
                        param_grid = param_test,n_jobs=40,cv=5)
...
for j in range(K):
    ...
    best_xgb = XGBClassifier(learning_rate=0.1, n_estimators=100, max_depth=best_max_depth, gamma=best_gamma,
                             ...
                             min_child_weight=best_min_child_weight, objective='binary:logistic', nthread=40,
