...
from scorecard_functions_V4 import *
...
for col in long_features:
    ...
    trainData[col1] = trainData[col].map(lambda x: AssignBin(x, cutOffPoints, special_attribute=[]))
...
display(sm.Logit(y, trainData['minPayL3M_Bin_WOE']).fit().params)  # -0.825956
display(sm.Logit(y, trainData['M2FreqL1M_WOE']).fit().params)  # -29.060739
