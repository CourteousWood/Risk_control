...
x_train = allData[allFeatures].astype(np.float32)
y_train = allData['target'].T.astype(np.float32)
...
for idx, col_name in enumerate(x_train.columns):
    col_dict[col_name] = 'col_'+str(idx)
...
# add L1 with 15 neurons 
dnn.add(tf.keras.layers.Dense(units=16,activation='swish'))
# add L2 witrh 12 neurons
dnn.add(tf.keras.layers.Dense(units=32,activation='swish'))
# add L3 with 10 neurons
dnn.add(tf.keras.layers.Dense(units=16,activation='swish'))

# dnn.add(tf.keras.layers.Dropout(0.75))

# add output layer
dnn.add(tf.keras.layers.Dense(units=1,activation='sigmoid'))

# compile the code
dnn.compile(optimizer='adam',loss='binary_crossentropy',metrics=['accuracy'],loss_weights=[0.1,100])
# fit 
dnn.fit(x_train,y_train,epochs=25,batch_size=30,validation_data=(x_validation,y_validation))
...
# Model Predict
y_pred = dnn.predict(x_validation,batch_size=25)
ypred = (y_pred>0.5)
...
print(confusion_matrix(y_validation,ypred))
print(accuracy_score(y_validation,ypred))

# In[182]:


roc_auc_score(y_validation,ypred)
...
#                           dropout = 0.5
#                         )
# clf.train(input_fn=lambda:input_fn(x_train,y_train,True),steps = 10)

# y_predict = list(clf.predict(input_fn=lambda:df_to_dataset(x_validation,None,False)))

# xxxx = [i['probabilities'] for i in y_predict]
# xxxxx = [0 if i[0] >0.5 else 1 for i in xxxx]

# auc_score = roc_auc_score(y_validation,xxxxx)
...
for dropout_prob in np.linspace(0,0.99,20):
    print("the current choise of hidden units number is {}".format(dropout_prob))
    clf=tf.estimator.DNNClassifier(feature_columns = my_feature_columns,
...
best_dropout_prob = max(dropout_selection.items(), key=operator.itemgetter(1))[0]   #80
